...
static int sensor_chip_init(struct i2c_client *client)
{
	...
	pr_info("Function: sensor_chip_init");
...
// 轮询模式的回调函数，通过 delayed work 实现周期读取传感器数据
static void  sensor_delaywork_func(struct work_struct *work)
{
	...
	pr_info("Function: sensor_delaywork_func.");
	...
	// 调度下一次延时任务（通过在延时任务中再次调度延时任务，实现轮询）
...
static int sensor_irq_init(struct i2c_client *client)
{
	...
	pr_info("Function: sensor_irq_init.");

	// 中断模式初始化（如果设备树设置了 irq_enable=1，启用中断）并（根据 irq_flags 决定中断类型）
	...
	// 否则设置为轮询模式（irq_enable=0）
	} else if (!sensor->pdata->irq_enable) {
		
		pr_info("start first sensor_delaywork_func.");

		// 初始化延时工作，关联回调函数 sensor_delaywork_func()
		INIT_DELAYED_WORK(&sensor->delaywork, sensor_delaywork_func);
		sensor->stop_work = 1;
		// 设置轮询延迟（如果默认延迟小于 0，则改到 30）
		if (sensor->pdata->poll_delay_ms <= 0)
			sensor->pdata->poll_delay_ms = 30;
...
static int sensor_enable(struct sensor_private_data *sensor, int enable)
{
	...
	pr_info("Function: sensor_enable.");
			  ...
			  unsigned int cmd, unsigned long arg)
{
	...
	pr_info("Function: gsensor_dev_ioctl.");
...
static long compass_dev_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	pr_info("Function: compass_dev_compat_ioctl.");
...
{
	pr_info("gyro sensor open success.");
	return 0;
}
			  ...
			  unsigned int cmd, unsigned long arg)
{
	...
	pr_info("Function: gyro_dev_ioctl.");
	...
	case L3G4200D_IOCTL_SET_ENABLE:
		pr_info("case L3G4200D_IOCTL_SET_ENABLE.");
...
static int sensor_misc_device_register(struct sensor_private_data *sensor, int type)
{
	...
	pr_info("Function: sensor_misc_device_register.");
	pr_info("type = %d", type);
	...
	case SENSOR_TYPE_ACCEL:
		if (!sensor->ops->misc_dev) {
			pr_info("register accel sensor.");    // yes
			sensor->fops.owner = THIS_MODULE;
	...
	case SENSOR_TYPE_GYROSCOPE:
		if (!sensor->ops->misc_dev) {
			pr_info("register gyro sensor.");    // yes
			sensor->fops.owner = THIS_MODULE;
			...
			struct sensor_operate *ops)
{
	...
	pr_info("Function: sensor_register_device.");
	...
	// gsensor_qmi8658 1-006b: sensor_register_device: qmi8658_acc, id = 37
